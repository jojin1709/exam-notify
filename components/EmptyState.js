export default function EmptyState({ query, category }) {
  return (
    <div className="text-center py-10 sm:py-16 px-4 sm:px-6">
      ...
      <h3 className="text-base sm:text-lg font-bold text-navy-900 mb-1.5 sm:mb-2">No results found</h3>
      <p className="text-navy-500 text-xs sm:text-sm max-w-sm mx-auto leading-relaxed">
        {query ? (
          <>No notifications match &ldquo;<span className="font-semibold text-navy-700">{query}</span>&rdquo;. Try different search.</>
        ) : (
